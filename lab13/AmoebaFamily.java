...
public class AmoebaFamily implements Iterable<AmoebaFamily.Amoeba> {
    ...
    public String longestName() {
        if (root != null) {
            return this.longestNameHelper(root);
        }
        return "";
    }

    public static String longestNameHelper(Amoeba current) {
        String longest = current.name;
        for (Amoeba a : current.children) {
            if (longest.length() < a.name.length()) {
                longest = a.name;
            }
        }
        return longest;
