public class BinaryTree<T> {
    ...
    public int heightHelper(TreeNode<T> current) {
        ...
        TreeNode<T> tallest = current.left;
        if (heightHelper(current.left) < heightHelper(current.right)) {
            tallest = current.right;
        }
        return 1 + heightHelper(tallest);
    }
    ...
    private TreeNode<Integer> fibTreeHelper(int N) {
        if (N == 1) {
        ...
        return new TreeNode<Integer>(left.item + right.item, left, right);
    }
